...
describe SportsApi::Fetcher::Boxscore::NCF do
  describe '#find' do
    ...
    before { expect_instance.to receive(:scoreboard).with('football', 'college-football', week: 1, limit: 300, groups: 80).and_return(json_stub_week_1) }
    before { expect_instance.to receive(:scoreboard).with('football', 'college-football', week: 1, limit: 300, groups: 80).and_return(json_stub_week_1) }
    before { expect_any_instance_of(SportsApi::Fetcher::Boxscore::NCF).to receive(:scoreboard).with('ncf/game', gameId: gameid).and_return(html_stub) }
